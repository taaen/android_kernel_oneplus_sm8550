			  ...
			  int (*open)(struct inode *, struct file *))
{
	...
	if (f->f_mode & FMODE_WRITE) {
		/*
		 * Depends on the full fence from get_write_access() to
		 * synchronize against collapse_file() regarding i_writecount
		 * and nr_thps updates. Ensures subsequent insertion of THPs
		 * into the page cache will fail.
		 */
