...
retry:
	...
	if (!S_ISDIR(inode->i_mode))
		goto same;
	/*
	 * In restrict mode, fsync() always tries to trigger checkpoint for
	 * metadata consistency. In other modes, trigger checkpoint only when
	 * the target file's parent had its xattr metadata updated.
	 */
